...
use App\Http\Resources\Api\V1\SoccerGinasiumResources;
use App\Models\SoccerGinasium;
use App\Services\Api\V1\SoccerGinasiumService;
use App\Traits\HttpResponses;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;

class SoccerGinasiumController extends Controller
{
    use HttpResponses;
    ...
    {
        $services = (new SoccerGinasiumService())
            ->setModel(SoccerGinasium::class)
            ->index();

        if ($services['status'] == 200) {
            return $this->response('Success', $services['status'], SoccerGinasiumResources::collection($services['data']));
        } else {
    ...
    {
        $service = (new SoccerGinasiumService())
            ->setModel(SoccerGinasium::class)
            ->show($id);

        if ($service['status'] == 200) {
            return $this->response('Success', $service['status'], new SoccerGinasiumResources($service['data']));
        } else {
    ...
    {
        $this->validData();
        $created = new SoccerGinasiumResources(
            (new SoccerGinasiumService())
                ->setModel(SoccerGinasium::class)
    ...
    {
        $this->validData();
        $updated = new SoccerGinasiumResources(
            (new SoccerGinasiumService())
                ->setModel(SoccerGinasium::class)
    ...
    {
        $deleted = (new SoccerGinasiumService())
            ->setModel(SoccerGinasium::class)
            ->destroy($id);
